  ...
  $.HSCore.components.HSCountdown = {
    ...
    initCountdowns: function () {
      ...
      this.collection.each(function (i, el) {
        var $this = $(el),
          options = {
            endDate: $this.data("end-date")
              ? new Date($this.data("end-date"))
              : new Date(),
            startDate: $this.data("start-date")
              ? new Date($this.data("start-date"))
              : new Date(),
        ...
        if (self.config["circles"] && $this.data("start-date"))
          self._initPiesImplementation($this, options);
    ...
    _initPiesImplementation: function (container, options) {
      ...
      if (options.yearsEl.length) {
        self._preparePieItem(options.yearsEl, {
          maxValue:
            options.endDate.getFullYear() - options.startDate.getFullYear(),
      ...
      if (options.daysEl.length) {
        self._preparePieItem(options.daysEl, {
          maxValue: self._getDaysMaxValByFormat(
            options.daysFormat,
            options.startDate,
            options.endDate
          ),
      ...
      container.countdown(options.endDate, function (e) {
        ...
        if (options.yearsEl.length) {
          options.yearsEl
            .data("circle")
            .update(e.strftime(options.yearsFormat));
        }
        ...
        if (options.monthsEl.length) {
          options.monthsEl
            .data("circle")
            .update(e.strftime(options.monthsFormat));
        }
        ...
        if (options.hoursEl.length) {
          options.hoursEl
            .data("circle")
            .update(e.strftime(options.hoursFormat));
        }
        ...
        if (options.minutesEl.length) {
          options.minutesEl
            .data("circle")
            .update(e.strftime(options.minutesFormat));
        }
        ...
        if (options.secondsEl.length) {
          options.secondsEl
            .data("circle")
            .update(e.strftime(options.secondsFormat));
        }
      });
    },
    ...
    _preparePieItem: function (el, options) {
      ...
      if (options["font-size"]) {
        el.find("." + self.config["textClass"]).css(
          "font-size",
